...
#data[which(data[,1] == data$grupa),i]<-impute(data[which(data[,1] == data$grupa),i],median(data[which(data[,1] == data$grupa),i],na.rm=TRUE))
...
datapodzial <- split(data, data[1])#podzial po 1 grupie uwzglednic w manual
...
#pvalue musi byc większe od 0.05
par(mfrow = c(1, 1))
...
folder <- getwd()
cat("Plik z raportem z przeprowadzonej analizy ('ProjektBartoszKasprzak.txt'), a także plik pdf z wszytskimi wykresami ('ProjektBKwykresy.pdf') znajdziesz w folderze: ", folder)
dev.off()
